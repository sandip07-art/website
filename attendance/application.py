...
class Timer:
    ...
    def lecture_time(self):
        ...
        for entry in self.timing_list:
            ...
            # Use <= for start to include exact hour times
...
class Utility:
    ...
    def export_attendance(self, attendance):
        ...
        # 24/7 Mode: Create session-based subject name
        current_time = datetime.now()
        subject = f"24/7 Session - {current_time.strftime('%Y-%m-%d %H:%M')}"
        ...
        if self.attribute.host_faculty and 'Email' in self.attribute.host_faculty:
    ...
    def college_over(self):
        # Always enable monitor - allow 24/7 attendance
        self.console_output('24/7 Attendance Mode - Monitor always available')
        self.console_output(f'Current time: {datetime.now().strftime("%H:%M:%S")}')
    ...
    def is_holiday(self):
        ...
        # 24/7 Mode: Always allow monitoring regardless of day
        self.console_output('24/7 Mode: All days are working days')
        self.console_output(f'Current day: {datetime.now().strftime("%A")}')
        
        # Enable all buttons for 24/7 operation
...
class Monitor(Utility):
    ...
    def monitor_cam(self):
        ...
        # Setup video capture stream from specified capture device with better error handling
        self.console_output(f'Attempting to open camera device: {self.capture_device}')
        self.capture = cv2.VideoCapture(self.capture_device, cv2.CAP_DSHOW)
        
        # Check if camera opened successfully
        if not self.capture.isOpened():
            self.console_output('Failed to open camera with DirectShow backend. Trying default backend...')
            self.capture = cv2.VideoCapture(self.capture_device)
            
        if not self.capture.isOpened():
            self.console_output('Failed to open camera with default backend. Trying MSMF backend...')
            self.capture = cv2.VideoCapture(self.capture_device, cv2.CAP_MSMF)
            
        if not self.capture.isOpened():
            self.console_output('ERROR: Could not access camera! Please check:')
            self.console_output('1. Camera is connected and working')
            self.console_output('2. No other application is using the camera')
            self.console_output('3. Camera permissions are enabled')
            self.monitor = False
            return
        ...
        if ret and test_frame is not None:
            self.console_output(f'✅ Camera initialized successfully! Frame size: {test_frame.shape}')
        else:
            self.console_output('❌ Camera opened but cannot read frames')
            self.monitor = False
            return
        ...
        while self.monitor:
            ...
            if not ret or frame is None:
                self.console_output('Failed to read frame from camera')
                continue
            frame = cv2.flip(frame, 1)
            
            # Debug: Add timestamp to frame
            timestamp = datetime.now().strftime("%H:%M:%S")
            cv2.putText(frame, f"Camera Active - {timestamp}", (10, 30), 
            ...
            try:
                ...
                # Debug: Log successful frame conversion
                if not hasattr(self, 'frame_count'):
                    self.frame_count = 0
                self.frame_count += 1
                if self.frame_count % 30 == 0:  # Log every 30 frames
                    self.console_output(f'Camera frames processed: {self.frame_count}')
                    
            except Exception as e:
                self.console_output(f'Error converting frame to QImage: {str(e)}')
                continue
    ...
    # Test camera access before starting monitor
    def test_camera_access(self):
        """Test if camera is accessible before starting monitor"""
        test_capture = cv2.VideoCapture(self.capture_device, cv2.CAP_DSHOW)
        if not test_capture.isOpened():
            test_capture = cv2.VideoCapture(self.capture_device)
        if not test_capture.isOpened():
            test_capture = cv2.VideoCapture(self.capture_device, cv2.CAP_MSMF)
    ...
    def monitor_trigger(self):
        ...
        if self.cam_on:
            ...
            if not self.test_camera_access():
                self.console_output('Camera test failed! Please check camera connection.')
                return
    ...
    def time_check(self):
        ...
        # 24/7 Mode: Always allow attendance monitoring
        # Create a continuous lecture session for 24/7 operation
        current_time = datetime.now()
        
        # Set lecture end time to 1 hour from now (continuous sessions)
        self.attribute.lecture_end = current_time + timedelta(hours=1)
        self.attribute.lecture_number = 0  # Single continuous session
        self.attribute.lecture_time = [current_time.strftime("%H:%M"), (current_time + timedelta(hours=1)).strftime("%H:%M")]
        self.attribute.lecture_tuple = tuple(self.attribute.lecture_time)
        
        # Reset warning flag for continuous operation
